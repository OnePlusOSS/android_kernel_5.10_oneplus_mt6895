...
static const struct mt6375_chg_field mt6375_chg_fields[F_MAX] = {
	...
	MT6375_CHG_FIELD(F_CHRDET_OV, MT6375_REG_CHRDET_STAT, 1, 1),
	MT6375_CHG_FIELD(F_CHRDET_UV, MT6375_REG_CHRDET_STAT, 0, 0),
};
...
/*#ifdef OPLUS_FEATURE_CHG_BASIC*/
static int to_psy_status(struct mt6375_chg_data *ddata)
{
	int ret = 0;
	u32 stat;
	bool chg_en = false, pwr_rdy = false;

	pwr_rdy = atomic_read(&ddata->attach);
	ret = pwr_rdy | mt6375_chg_is_enabled(ddata, &chg_en);
...
static irqreturn_t mt6375_int_chrdet_ov_handler(int irq, void *data)
{
	struct mt6375_chg_data *ddata = data;
	int ret;
	u32 val_ov;
	...
	mt_dbg(ddata->dev, "mt6375_int_chrdet_ov_handler ++\n");

	ret = mt6375_chg_field_get(data, F_CHRDET_OV, &val_ov);
	if (ret < 0)
		return ret;
	mt_dbg(ddata->dev, "val_ov = %d\n", val_ov);
...
static irqreturn_t mt6375_int_chrdet_ext_handler(int irq, void *data)
{
	...
	mt_dbg(ddata->dev, "mt6375_int_chrdet_ext_handler ++\n");
	ret = mt6375_chg_field_get(data, F_CHRDET_EXT, &val_ext);
	if (ret < 0)
		return ret;
	mt_dbg(ddata->dev, "val_ext = %d\n", val_ext);
	...
	ret = mt6375_chg_field_get(data, F_CHRDET_UV, &val_uv);
	if (ret < 0)
		return ret;
	mt_dbg(ddata->dev, "val_uv = %d\n", val_uv);

	if (!val_ext) {
		if (val_ov) {
			mt_dbg(ddata->dev, "chrdet ov but not notify dettach\n");
			return IRQ_HANDLED;
		}
		mt_dbg(ddata->dev, "chrdet dettach\n");
		/*#ifdef OPLUS_FEATURE_CHG_BASIC*/
