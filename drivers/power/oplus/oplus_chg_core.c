...
{
	size_t last_module = (size_t)&__oplus_chg_module_end -
			    sizeof(struct oplus_chg_module);

	if (&__oplus_chg_module_start == &__oplus_chg_module_end)
		return (struct oplus_chg_module *)&__oplus_chg_module_start;
	else
		return (struct oplus_chg_module *)last_module;
}
#endif /* MODULE */
...
#endif
#if IS_ENABLED(CONFIG_OPLUS_CHG_V2)
