...
int __attribute__((weak)) oplus_voocphy_enable(bool enable)
{
	return 0;
}

int __attribute__((weak)) oplus_voocphy_reset_again(void)
{
	return 0;
}
...
void __attribute__((weak)) oplus_chg_set_curr_level_to_voocphy(void)
{

}

void __attribute__((weak)) oplus_chg_set_match_temp_to_voocphy(void)
{
...
int mp2650_set_vindpm_vol(int vol)//default 4.5V
{
	...
	chg_err("mp2650_set_vindpm_vol %d\n", vol);
...
int mp2650_enable_charging(void)
{
	...
	if(oplus_is_rf_ftm_mode()) {
		chg_err("is rf_ftm_mode, mp2650 not enable_charging\n");
		return 0;
	}
...
int mp2650_unsuspend_charger(void)
{
	...
	if(oplus_is_rf_ftm_mode()) {
		chg_err("is rf_ftm_mode, mp2650 not unsuspend_charger\n");
		return 0;
	}
...
EXPORT_SYMBOL(mp2650_otg_enable);
...
EXPORT_SYMBOL(mp2650_otg_disable);
...
int mp2650_hardware_init(void)
{
    ...
    if(oplus_is_rf_ftm_mode()) {
        ...
        pr_err("[OPLUS_CHG] is_rf_ftm_mode, suspend charger \n");
    }
...
EXPORT_SYMBOL(oplus_get_chg_ops);
...
static int oplus_wrx_otg_en_gpio_init(struct chip_mp2650 *chip)
{
	if (!chip) {
	...
	chip->wrx_otg_en_pinctrl = devm_pinctrl_get(chip->dev);
	if (IS_ERR_OR_NULL(chip->wrx_otg_en_pinctrl)) {
		chg_err("get wrx_otg_en_pinctrl fail\n");
		return -EINVAL;
	}

	chip->wrx_otg_en_active =
		pinctrl_lookup_state(chip->wrx_otg_en_pinctrl, "wrx_otg_en_active");
	...
	chip->wrx_otg_en_sleep =
		pinctrl_lookup_state(chip->wrx_otg_en_pinctrl, "wrx_otg_en_sleep");
	...
	pinctrl_select_state(chip->wrx_otg_en_pinctrl, chip->wrx_otg_en_sleep);
...
void oplus_set_wrx_otg_en_val(int value)
{
	...
	if (value) {
		...
		pinctrl_select_state(chip->wrx_otg_en_pinctrl, chip->wrx_otg_en_active);
	} else {
		gpio_direction_output(chip->wrx_otg_en_gpio, 0);
		pinctrl_select_state(chip->wrx_otg_en_pinctrl, chip->wrx_otg_en_sleep);
	}
...
int oplus_boost_en_gpio_init(struct chip_mp2650 *chip)
{
	if (!chip) {
		chg_err("chip_mp2650 not ready!\n");
		return -EINVAL;
	}

	chip->boost_en_pinctrl = devm_pinctrl_get(chip->dev);
	if (IS_ERR_OR_NULL(chip->boost_en_pinctrl)) {
		chg_err("get boost_en_pinctrl fail\n");
		return -EINVAL;
	}

	chip->boost_en_active = pinctrl_lookup_state(chip->boost_en_pinctrl, "boost_en_active");
	if (IS_ERR_OR_NULL(chip->boost_en_active)) {
		chg_err("get boost_en_active fail\n");
		return -EINVAL;
	}

	chip->boost_en_sleep = pinctrl_lookup_state(chip->boost_en_pinctrl, "boost_en_sleep");
	if (IS_ERR_OR_NULL(chip->boost_en_sleep)) {
		chg_err("get boost_en_sleep fail\n");
		return -EINVAL;
	}

	gpio_direction_output(chip->boost_en_gpio, 0);
	pinctrl_select_state(chip->boost_en_pinctrl,  chip->boost_en_sleep);
	return 0;
}
...
void oplus_set_boost_en_val(int value)
{
	...
	if (chip->boost_en_gpio <= 0) {
		chg_err("boost_en_gpio not exist, return\n");
		return;
	}

	if (IS_ERR_OR_NULL(chip->boost_en_pinctrl)
			|| IS_ERR_OR_NULL(chip->boost_en_active)
			|| IS_ERR_OR_NULL(chip->boost_en_sleep)) {
		chg_err("boost_en pinctrl null, return\n");
		return;
	}

	if (value) {
		gpio_direction_output(chip->boost_en_gpio , 1);
		pinctrl_select_state(chip->boost_en_pinctrl, chip->boost_en_active);
	} else {
		gpio_direction_output(chip->boost_en_gpio, 0);
		pinctrl_select_state(chip->boost_en_pinctrl, chip->boost_en_sleep);
	}

	chg_err("set value:%d, gpio_val:%d\n", value, gpio_get_value(chip->boost_en_gpio));
}
EXPORT_SYMBOL(oplus_set_boost_en_val);

int oplus_ext1_otg_en_gpio_init(struct chip_mp2650 *chip)
{
	if (!chip) {
		chg_err("chip_mp2650 not ready!\n");
		return -EINVAL;
	...
	chip->ext1_otg_en_pinctrl = devm_pinctrl_get(chip->dev);
	if (IS_ERR_OR_NULL(chip->ext1_otg_en_pinctrl)) {
		chg_err("get ext1_otg_en_pinctrl fail\n");
		return -EINVAL;
	}

	chip->ext1_otg_en_active = pinctrl_lookup_state(chip->ext1_otg_en_pinctrl, "ext1_otg_en_active");
	if (IS_ERR_OR_NULL(chip->ext1_otg_en_active)) {
		chg_err("get ext1_otg_en_active fail\n");
		return -EINVAL;
	}

	chip->ext1_otg_en_sleep = pinctrl_lookup_state(chip->ext1_otg_en_pinctrl, "ext1_otg_en_sleep");
	if (IS_ERR_OR_NULL(chip->ext1_otg_en_sleep)) {
		chg_err("get ext1_otg_en_sleep fail\n");
		return -EINVAL;
	}

	gpio_direction_output(chip->ext1_otg_en_gpio, 0);
	pinctrl_select_state(chip->ext1_otg_en_pinctrl,  chip->ext1_otg_en_sleep);
	return 0;
}
...
void oplus_set_ext1_otg_en_val(int value)
{
	...
	if (chip->ext1_otg_en_gpio <= 0) {
		chg_err("ext1_otg_en_gpio not exist, return\n");
		return;
	}

	if (IS_ERR_OR_NULL(chip->ext1_otg_en_pinctrl)
			|| IS_ERR_OR_NULL(chip->ext1_otg_en_active)
			|| IS_ERR_OR_NULL(chip->ext1_otg_en_sleep)) {
		chg_err("ext1_otg_en pinctrl null, return\n");
		return;
	}

	if (value) {
		gpio_direction_output(chip->ext1_otg_en_gpio , 1);
		pinctrl_select_state(chip->ext1_otg_en_pinctrl, chip->ext1_otg_en_active);
	} else {
		gpio_direction_output(chip->ext1_otg_en_gpio, 0);
		pinctrl_select_state(chip->ext1_otg_en_pinctrl, chip->ext1_otg_en_sleep);
	}

	chg_err("set value:%d, gpio_val:%d\n", value, gpio_get_value(chip->ext1_otg_en_gpio));
}
EXPORT_SYMBOL(oplus_set_ext1_otg_en_val);

int oplus_ext2_otg_en_gpio_init(struct chip_mp2650 *chip)
{
	if (!chip) {
		chg_err("chip_mp2650 not ready!\n");
		return -EINVAL;
	...
	chip->ext2_otg_en_pinctrl = devm_pinctrl_get(chip->dev);
	if (IS_ERR_OR_NULL(chip->ext2_otg_en_pinctrl)) {
		chg_err("get ext2_otg_en_pinctrl fail\n");
		return -EINVAL;
	}

	chip->ext2_otg_en_active = pinctrl_lookup_state(chip->ext2_otg_en_pinctrl, "ext2_otg_en_active");
	if (IS_ERR_OR_NULL(chip->ext2_otg_en_active)) {
		chg_err("get ext2_otg_en_active fail\n");
		return -EINVAL;
	}

	chip->ext2_otg_en_sleep = pinctrl_lookup_state(chip->ext2_otg_en_pinctrl, "ext2_otg_en_sleep");
	if (IS_ERR_OR_NULL(chip->ext2_otg_en_sleep)) {
		chg_err("get ext2_otg_en_sleep fail\n");
		return -EINVAL;
	}

	gpio_direction_output(chip->ext2_otg_en_gpio, 0);
	pinctrl_select_state(chip->ext2_otg_en_pinctrl,  chip->ext2_otg_en_sleep);
	return 0;
}
...
void oplus_set_ext2_otg_en_val(int value)
{
	...
	if (chip->ext2_otg_en_gpio <= 0) {
		chg_err("ext2_otg_en_gpio not exist, return\n");
		return;
	}

	if (IS_ERR_OR_NULL(chip->ext2_otg_en_pinctrl)
			|| IS_ERR_OR_NULL(chip->ext2_otg_en_active)
			|| IS_ERR_OR_NULL(chip->ext2_otg_en_sleep)) {
		chg_err("ext2_otg_en pinctrl null, return\n");
		return;
	}

	if (value) {
		gpio_direction_output(chip->ext2_otg_en_gpio , 1);
		pinctrl_select_state(chip->ext2_otg_en_pinctrl, chip->ext2_otg_en_active);
	} else {
		gpio_direction_output(chip->ext2_otg_en_gpio, 0);
		pinctrl_select_state(chip->ext2_otg_en_pinctrl, chip->ext2_otg_en_sleep);
	}

	chg_err("set value:%d, gpio_val:%d\n", value, gpio_get_value(chip->ext2_otg_en_gpio));
}
EXPORT_SYMBOL(oplus_set_ext2_otg_en_val);

#if 0
static bool is_support_tx_boost(struct chip_mp2650 *chip)
{
	if (!chip) {
		printk(KERN_ERR "[OPLUS_CHG][%s]: mp2650 not ready!\n", __func__);
		return false;
	}

	if (gpio_is_valid(chip->tx_boost_en_gpio))
		return true;
...
static int oplus_tx_boost_en_gpio_init(struct chip_mp2650 *chip)
{
	if (!chip) {
		printk(KERN_ERR "[OPLUS_CHG][%s]: mp2650 not ready!\n", __func__);
		return -EINVAL;
	...
	chip->tx_boost_en_pinctrl = devm_pinctrl_get(chip->dev);
	if (IS_ERR_OR_NULL(chip->tx_boost_en_pinctrl)) {
		chg_err("get tx_boost_en_pinctrl fail\n");
		return -EINVAL;
	}

	chip->tx_boost_en_active =
		pinctrl_lookup_state(chip->tx_boost_en_pinctrl, "tx_boost_en_active");
	if (IS_ERR_OR_NULL(chip->tx_boost_en_active)) {
		chg_err("get tx_boost_en_active fail\n");
		return -EINVAL;
	}

	chip->tx_boost_en_sleep =
		pinctrl_lookup_state(chip->tx_boost_en_pinctrl, "tx_boost_en_sleep");
	if (IS_ERR_OR_NULL(chip->tx_boost_en_sleep)) {
		chg_err("get tx_boost_en_sleep fail\n");
		return -EINVAL;
	}

	gpio_direction_output(chip->tx_boost_en_gpio, 0);
	pinctrl_select_state(chip->tx_boost_en_pinctrl, chip->tx_boost_en_sleep);

	return 0;
}

void oplus_set_tx_boost_en_val(int value)
{
	struct chip_mp2650 *chip = charger_ic;
	...
	if (chip->tx_boost_en_gpio <= 0) {
		chg_err("tx_boost_en_gpio not exist, return\n");
		return;
	}

	if (IS_ERR_OR_NULL(chip->tx_boost_en_pinctrl)
		|| IS_ERR_OR_NULL(chip->tx_boost_en_active)
		|| IS_ERR_OR_NULL(chip->tx_boost_en_sleep)) {
		chg_err("otg_boost_en pinctrl null, return\n");
		return;
	}

	if (value) {
		gpio_direction_output(chip->tx_boost_en_gpio , 1);
		pinctrl_select_state(chip->tx_boost_en_pinctrl, chip->tx_boost_en_active);
	} else {
		gpio_direction_output(chip->tx_boost_en_gpio, 0);
		pinctrl_select_state(chip->tx_boost_en_pinctrl, chip->tx_boost_en_sleep);
	}

	chg_err("set value:%d, gpio_val:%d\n", value, gpio_get_value(chip->tx_boost_en_gpio));
}

static int oplus_tx_ovp_en_gpio_init(struct chip_mp2650 *chip)
{
	if (!chip) {
		printk(KERN_ERR "[OPLUS_CHG][%s]: bcdev not ready!\n", __func__);
		return -EINVAL;
	...
	chip->tx_ovp_en_pinctrl = devm_pinctrl_get(chip->dev);
	if (IS_ERR_OR_NULL(chip->tx_ovp_en_pinctrl)) {
		chg_err("get tx_ovp_en_pinctrl fail\n");
		return -EINVAL;
	}

	chip->tx_ovp_en_active =
		pinctrl_lookup_state(chip->tx_ovp_en_pinctrl, "tx_ovp_en_active");
	if (IS_ERR_OR_NULL(chip->tx_ovp_en_active)) {
		chg_err("get tx_ovp_en_active fail\n");
		return -EINVAL;
	}

	chip->tx_ovp_en_sleep =
		pinctrl_lookup_state(chip->tx_ovp_en_pinctrl, "tx_ovp_en_sleep");
	if (IS_ERR_OR_NULL(chip->tx_ovp_en_sleep)) {
		chg_err("get tx_ovp_en_sleep fail\n");
		return -EINVAL;
	}

	gpio_direction_output(chip->tx_ovp_en_gpio, 0);
	pinctrl_select_state(chip->tx_ovp_en_pinctrl, chip->tx_ovp_en_sleep);

	return 0;
}

void oplus_set_tx_ovp_en_val(int value)
{
	struct chip_mp2650 *chip = charger_ic;
	...
	if (chip->tx_ovp_en_gpio <= 0) {
		chg_err("tx_ovp_en_gpio not exist, return\n");
		return;
	}

	if (IS_ERR_OR_NULL(chip->tx_ovp_en_pinctrl)
		|| IS_ERR_OR_NULL(chip->tx_ovp_en_active)
		|| IS_ERR_OR_NULL(chip->tx_ovp_en_sleep)) {
		chg_err("otg_ovp_en pinctrl null, return\n");
		return;
	}

	if (value) {
		gpio_direction_output(chip->tx_ovp_en_gpio , 1);
		pinctrl_select_state(chip->tx_ovp_en_pinctrl, chip->tx_ovp_en_active);
	} else {
		gpio_direction_output(chip->tx_ovp_en_gpio, 0);
		pinctrl_select_state(chip->tx_ovp_en_pinctrl, chip->tx_ovp_en_sleep);
	}

	chg_err("set value:%d, gpio_val:%d\n", value, gpio_get_value(chip->tx_ovp_en_gpio));
}
...
static int mp2650_gpio_init(struct chip_mp2650 *chip)
{
	...
	else
	{
		...
		chg_err("chip->mps_otg_en_gpio =%d\n",chip->mps_otg_en_gpio);
	}

	chip->tx_boost_en_gpio = of_get_named_gpio(node, "qcom,tx_boost_en-gpio", 0);
	if (chip->tx_boost_en_gpio <= 0) {
		chg_err("Couldn't read qcom,tx_boost_en-gpio, rc = %d\n", rc);
	} else {
		if (gpio_is_valid(chip->tx_boost_en_gpio)) {
			rc = gpio_request(chip->tx_boost_en_gpio, "tx_boost_en-gpio");
			if (rc) {
				chg_err("unable to tx_boost_en-gpio:%d\n", chip->tx_boost_en_gpio);
			} else {
				rc = oplus_tx_boost_en_gpio_init(chip);
				if (rc)
					chg_err("unable to init tx_boost_en-gpio:%d\n", chip->tx_boost_en_gpio);
				else
					chg_err("init tx_boost_en_gpio level[%d]\n", gpio_get_value(chip->tx_boost_en_gpio));
			}
		}
		chg_err("tx_boost_en-gpio:%d\n", chip->tx_boost_en_gpio);
	}

	chip->tx_ovp_en_gpio = of_get_named_gpio(node, "qcom,tx_ovp_en-gpio", 0);
	if (chip->tx_ovp_en_gpio <= 0) {
		chg_err("Couldn't read qcom,tx_ovp_en-gpio, rc = %d\n", rc);
	} else {
		if (gpio_is_valid(chip->tx_ovp_en_gpio)) {
			rc = gpio_request(chip->tx_ovp_en_gpio, "tx_ovp_en-gpio");
			if (rc) {
				chg_err("unable to tx_ovp_en-gpio:%d\n", chip->tx_ovp_en_gpio);
			} else {
				rc = oplus_tx_ovp_en_gpio_init(chip);
				if (rc)
					chg_err("unable to init tx_ovp_en-gpio:%d\n", chip->tx_ovp_en_gpio);
				else
					chg_err("init tx_ovp_en_gpio level[%d]\n", gpio_get_value(chip->tx_ovp_en_gpio));
			}
		}
	...
	if (chip->wrx_otg_en_gpio <= 0) {
		...
		if (gpio_is_valid(chip->wrx_otg_en_gpio)) {
			rc = gpio_request(chip->wrx_otg_en_gpio, "wrx_otg_en-gpio");
			if (rc) {
	...
	chip->boost_en_gpio = of_get_named_gpio(node, "qcom,boost_en-gpio", 0);
	if (chip->boost_en_gpio <= 0) {
		chg_err("Couldn't read qcom,boost_en-gpio rc=%d, qcom,boost_en-gpio:%d\n",
				rc, chip->boost_en_gpio);
	} else {
		if (gpio_is_valid(chip->boost_en_gpio)) {
			rc = gpio_request(chip->boost_en_gpio, "boost_en-gpio");
			if (rc) {
				...
				if(rc) {
					...
					chg_err("init boost_en_gpio level[%d]\n",
						gpio_get_value(chip->boost_en_gpio));
				}
			}
	...
	chip->ext1_otg_en_gpio = of_get_named_gpio(node, "qcom,ext1_otg_en-gpio", 0);
	if (chip->ext1_otg_en_gpio <= 0) {
		chg_err("Couldn't read qcom,ext1_otg_en-gpio rc=%d, qcom,ext1_otg_en-gpio:%d\n",
				rc, chip->ext1_otg_en_gpio);
	} else {
		if (gpio_is_valid(chip->ext1_otg_en_gpio)) {
			rc = gpio_request(chip->ext1_otg_en_gpio, "ext1_otg_en-gpio");
			if (rc) {
				chg_err("unable to request ext1_otg_en_gpio:%d\n",
					chip->ext1_otg_en_gpio);
			} else {
				...
				if(rc) {
					...
					chg_err("init ext1_otg_en_gpio level[%d]\n",
						gpio_get_value(chip->ext1_otg_en_gpio));
				}
			}
	...
	chip->ext2_otg_en_gpio = of_get_named_gpio(node, "qcom,ext2_otg_en-gpio", 0);
	if (chip->ext2_otg_en_gpio <= 0) {
		chg_err("Couldn't read qcom,ext2_otg_en-gpio rc=%d, qcom,ext2_otg_en-gpio:%d\n",
				rc, chip->ext2_otg_en_gpio);
	} else {
		if (gpio_is_valid(chip->ext2_otg_en_gpio)) {
			rc = gpio_request(chip->ext2_otg_en_gpio, "ext2_otg_en-gpio");
			if (rc) {
				chg_err("unable to request ext2_otg_en_gpio:%d\n",
					chip->ext2_otg_en_gpio);
			} else {
				...
				if(rc) {
					...
					chg_err("init ext2_otg_en_gpio level[%d]\n",
						gpio_get_value(chip->ext2_otg_en_gpio));
				}
			}
...
static int mp2650_wls_input_enable(struct oplus_chg_ic_dev *ic_dev, bool en)
{
	if (!probe_done)
		return 0;
...
static int mp2650_wls_output_enable(struct oplus_chg_ic_dev *ic_dev, bool en)
{
	int rc = 0;
...
static int mp2650_wls_set_icl(struct oplus_chg_ic_dev *ic_dev, int icl_ma)
{
	int rc = 0;
	...
	if (oplus_get_wired_chg_present()) {
		return 0;
	}
...
static int mp2650_wls_set_fcc(struct oplus_chg_ic_dev *ic_dev, int fcc_ma)
{
	int rc = 0;
...
static int mp2650_wls_set_fv(struct oplus_chg_ic_dev *ic_dev, int fv_mv)
{
	int rc = 0;
...
static int mp2650_wls_rechg_vol(struct oplus_chg_ic_dev *ic_dev, int fv_mv)
{
	int rc = 0;
...
static int mp2650_wls_get_input_curr(struct oplus_chg_ic_dev *ic_dev, int *curr)
{
	int rc = 0;
...
static int mp2650_wls_get_input_vol(struct oplus_chg_ic_dev *ic_dev, int *vol)
{
	int rc = 0;
...
static int mp2650_wls_set_boost_en(struct oplus_chg_ic_dev *ic_dev, bool en)
{
	int rc = 0;
	struct chip_mp2650 *chip = charger_ic;
...
static int mp2650_wls_set_boost_vol(struct oplus_chg_ic_dev *ic_dev, int vol_mv)
{
	int rc = 0;
	struct chip_mp2650 *chip = charger_ic;
...
static int mp2650_wls_set_aicl_enable(struct oplus_chg_ic_dev *ic_dev, bool en)
{
	int rc = 0;
...
static int mp2650_wls_rerun_aicl(struct oplus_chg_ic_dev *ic_dev)
{
	int rc = 0;
...
static int mp2650_wls_set_vindpm(struct oplus_chg_ic_dev *ic_dev, int vindpm_mv)
{
	int rc = 0;
	int vol = 0;
...
static struct oplus_chg_ic_buck_ops mp2650_ic_ops = {
	.chg_set_input_enable = mp2650_wls_input_enable,
	.chg_set_output_enable = mp2650_wls_output_enable,
	.chg_set_icl = mp2650_wls_set_icl,
	.chg_set_fcc = mp2650_wls_set_fcc,
	.chg_set_fv = mp2650_wls_set_fv,
	.chg_set_rechg_vol = mp2650_wls_rechg_vol,
	.chg_get_icl = NULL,
	.chg_get_input_curr = mp2650_wls_get_input_curr,
	.chg_get_input_vol = mp2650_wls_get_input_vol,
	.chg_set_boost_en = mp2650_wls_set_boost_en,
	.chg_set_boost_vol = mp2650_wls_set_boost_vol,
	.chg_set_boost_curr_limit =  NULL,
	.chg_set_aicl_enable = mp2650_wls_set_aicl_enable,
	.chg_set_aicl_rerun = mp2650_wls_rerun_aicl,
	.chg_set_vindpm = mp2650_wls_set_vindpm,
};
...
static int mp2650_driver_probe(struct i2c_client *client, const struct i2c_device_id *id) 
{
	...
	ret = of_property_read_u32(chg_ic->dev->of_node, "oplus,ic_type", &ic_type);
	if (ret < 0) {
		pr_err("can't get ic type, ret=%d\n", ret);
	}

	ret = of_property_read_u32(chg_ic->dev->of_node, "oplus,ic_index", &ic_index);
	if (ret < 0) {
		pr_err("can't get ic index, ret=%d\n", ret);
	}

	chg_ic->ic_dev = devm_oplus_chg_ic_register(chg_ic->dev, chg_ic->dev->of_node->name, ic_index);
	if (!chg_ic->ic_dev) {
		ret = -ENODEV;
		pr_err("register %s error\n", chg_ic->dev->of_node->name);
		return ret;
	}

	chg_ic->ic_dev->dev_ops = &mp2650_ic_ops;
	chg_ic->ic_dev->type = ic_type;
	probe_done = true;
...
EXPORT_SYMBOL(mp2650_driver_init);
void mp2650_driver_exit(void)
{
	i2c_del_driver(&mp2650_i2c_driver);
}
EXPORT_SYMBOL(mp2650_driver_exit);
