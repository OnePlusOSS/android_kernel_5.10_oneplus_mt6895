...
*/
	...
	if (data == VOOC_NOTIFY_FAST_PRESENT) {
		...
		if (oplus_chg_get_bcc_curr_done_status() == BCC_CURR_DONE_REQUEST) {
			bcc_curr_count = bcc_curr_count + 1;
			if (bcc_curr_count > 1) {
		...
		if (oplus_vooc_get_reply_bits() == 7) {
			...
			chip->bcc_soc_range = chip->soc_range;
			chip->bcc_temp_range = chip->vooc_temp_cur_range;
...
EXPORT_SYMBOL(oplus_vooc_check_chip_is_null);
