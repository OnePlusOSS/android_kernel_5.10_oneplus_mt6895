...
struct pmic_temp_tz {
    int tz_id;
    struct shell_ntc_info *pmic_tz_info;
};

static int pmic_temp_parse_iio_channel(struct device *dev,
                    struct shell_ntc_info *pmic_info)
{
    int ret;
    struct pmic_tz_data *tz_data = pmic_info->pmic_data;
    int i = 0;
    LOG_INF("pmic_temp_parse_iio_channel in\n");
...
static int shell_ntc_parse_lookup_table(struct device *dev,
                    struct shell_ntc_info *ntc_info)
{
    struct device_node *np = dev->of_node;
    int num, ret;
    LOG_INF("shell_ntc_parse_lookup_table in\n");
    ...
    LOG_INF("shell_ntc_parse_lookup_table num = %d\n",num);
    if (num % 2) {
    ...
    LOG_INF("shell_ntc_parse_lookup_table lookup_table_num = %d\n",ntc_info->lookup_table_num);
    return 0;
}
...
static __s16 oplus_shell_thermistor_conver_temp(__s32 res)
{
    int i = 0;
    int asize = 0;
    __s32 res1 = 0, res2 = 0;
...
static __s16 oplus_ts_shell_volt_to_temp(__u32 dwvolt)
{
    __s32 tres;
    __u64 dwvcrich = 0;
    __s32 shell_tmp = -100;
    ...
    shell_tmp = oplus_shell_thermistor_conver_temp(tres);
...
static int pmic_get_temp(void *data, int *temp)
{
    ...
    struct shell_ntc_info *temp_info = pmic_tz->pmic_tz_info;
    ...
    ret = val;
    output = oplus_ts_shell_volt_to_temp(ret);
...
static int pmic_register_thermal_zones(struct shell_ntc_info *pmic_info)
{
    //struct pmic_tz_data *tz_data = pmic_info->pmic_data;
...
static int shell_temp_probe(struct platform_device *pdev)
{
    struct shell_ntc_info *pmic_info;
    int ret;
    ...
    pmic_info->sensor_num = 1;
    pmic_info->pmic_data = (struct pmic_tz_data *)of_device_get_match_data(&pdev->dev);

    ret = shell_ntc_parse_lookup_table(&pdev->dev, pmic_info);
    if (ret < 0)
        return ret;
    ...
    if (ret)
        return ret;
    LOG_INF("shell_temp_probe sucess \n");

    return 0;
}
static struct pmic_tz_data shell_tz_data[] = {
    [0] = {
        .iio_chan_name = "pmic_adc_channel2",
    },
};
static const struct of_device_id shell_temp_of_match[] = {
    {
        .compatible = "mediatek,shell-temp",
        .data = (void *)&shell_tz_data,
    },
    {},
};
MODULE_DEVICE_TABLE(of, shell_temp_of_match);
static struct platform_driver oplus_temp_driver = {
    .probe = shell_temp_probe,
    .driver = {
        .name = "shell-temp",
        .of_match_table = shell_temp_of_match,
    },
};
...
MODULE_DESCRIPTION("Mediatek pmic temp shell driver");
