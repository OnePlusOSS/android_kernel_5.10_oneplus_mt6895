...
struct pmic_temp_tz {
	int tz_id;
	struct oplus_ntc_info *pmic_tz_info;
};

static int pmic_temp_parse_iio_channel(struct device *dev,
					struct oplus_ntc_info *pmic_info)
{
	int ret;
...
static int oplus_ntc_parse_lookup_table(struct device *dev,
					struct oplus_ntc_info *ntc_info)
{
	struct device_node *np = dev->of_node;
	int num, ret;
	chg_err("oplus_ntc_parse_lookup_table in\n");
	...
	printk("oplus_ntc_parse_lookup_table num = %d\n",num);
	if (num % 2) {
	...
	printk("oplus_ntc_parse_lookup_table lookup_table_num = %d\n",ntc_info->lookup_table_num);
	chg_err("oplus_ntc_parse_lookup_table out\n");
...
static int pmic_get_temp(void *data, int *temp)
{
	...
	struct oplus_ntc_info *temp_info = pmic_tz->pmic_tz_info;
...
static int pmic_register_thermal_zones(struct oplus_ntc_info *pmic_info)
{
	//struct pmic_tz_data *tz_data = pmic_info->pmic_data;
...
static int oplus_temp_probe(struct platform_device *pdev)
{
	struct oplus_ntc_info *pmic_info;
	int ret;
	chg_err("oplus_temp_probe in 1\n");
	if (!pdev->dev.of_node) {
	...
	ret = oplus_ntc_parse_lookup_table(&pdev->dev, pmic_info);
	if (ret < 0)
		return ret;
	...
	if (ret)
		return ret;
	chg_err("oplus_temp_probe sucess \n");

	return 0;
}

static struct pmic_tz_data oplus_tz_data[] = {
	[0] = {
...
static struct pmic_tz_data oplus_tz_data_qqcandy[] = {
	[0] = {
		.iio_chan_name = "pmic_adc_channel1",
	},
	[1] = {
...
static struct pmic_tz_data oplus_tz_data_80w[] = {
	[2] = {
		.iio_chan_name = "pmic_adc_channel7",
	},
	[3] = {
...
static const struct of_device_id oplus_temp_of_match[] = {
	{
		.compatible = "mediatek,oplus-temp",
		.data = (void *)&oplus_tz_data,
	},
	{
		.compatible = "mediatek,oplus-temp-qqcandy",
		.data = (void *)&oplus_tz_data_qqcandy,
	},
	{
		.compatible = "mediatek,oplus-temp-21641",
		.data = (void *)&oplus_tz_data_80w,
	},
	{},
};
MODULE_DEVICE_TABLE(of, oplus_temp_of_match);

static struct platform_driver oplus_temp_driver = {
	.probe = oplus_temp_probe,
	.driver = {
		.name = "oplus-temp",
		.of_match_table = oplus_temp_of_match,
	},
};
