...
static void i2c_gpio_reset(struct mtk_i2c *i2c)
{
	int ret = 0;
		...
		if ((ret == 0) || (i2c->irq_stat & (I2C_HS_NACKERR | I2C_ACKERR))) {
			if (msgs->addr == FG_DEVICE_ADDR || msgs->addr == CHARGE_PUMP_DEVICE_ADDR) {
				...
				if (oplus_get_fg_device_type() == DEVICE_TYPE_ZY0602) {
					...
					if (err_count_for_reset >= 1 && err_count_for_reset < 10) {
						i2c_gpio_reset(i2c);
					} else {
			    ...
			    struct i2c_msg msgs[], int num)
{
	...
	if (adap->nr == 5 || adap->nr == 1) //xiongxing add for i2c trans delay, 2022/05/16
