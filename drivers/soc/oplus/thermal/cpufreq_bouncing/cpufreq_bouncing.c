...
void cb_update(struct cpufreq_policy *pol, u64 time)
{
	...
	int cpu, prev_level;
	bool min_over_target_freq, isolated, temp;
	 ...
	 */
	temp = min_over_target_freq || isolated;
	delta = temp ? 0 : time - cb->last_ts;
	...
	if (debug)
		pr_info("cpu %d update: ts now %llu last %llu last_update %llu delta %llu update_d %llu cur %u \
				acc %llu, cur_level %d min_over_target_freq %d isolated %d last_core_boost %d\n",
			cpu,
			NSEC_TO_MSEC(time),
