	...
	if (stage == BEFORE_DSI_POWERDOWN) {
		...
		if (m->hskew == SDC_MFR || m->hskew == SDC_ADFR) {
			if (m_vrefresh == 60) {
				...
				lcm_cmd_count = sizeof(fhd_timing_switch_cmd_sdc90) / sizeof(struct LCM_setting_table);
				panel_send_pack_hs_cmd(dsi_drv, fhd_timing_switch_cmd_sdc90, lcm_cmd_count, cb, NULL);
			} else if (m_vrefresh == 120) {
