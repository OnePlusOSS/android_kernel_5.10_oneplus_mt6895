...
#endif
/* temp modify for 51 don't work well */
				 ...
				 unsigned int level)
{
	...
	static unsigned last_backlight_level = 0;
	int i = 0;
...
//		bl_tb1[1] = 0x11;
	...
	/* temp modify for 51 don't work well */
	if ((0 == last_backlight_level) && (level > 1)) {
		for (i = 0; i < sizeof(lcm_display_on_setting)/sizeof(struct LCM_setting_table); i++){
			cb(dsi, handle, lcm_display_on_setting[i].para_list, lcm_display_on_setting[i].count);
			pr_info("resend 53_reg when wakeup\n");
		}
	}
#if defined(CONFIG_PXLW_IRIS)
...
#endif
	register_device_proc("lcd", "boe_nt37701a", "boe");
...
static struct mipi_dsi_driver jdi_driver = {
	...
	.remove = jdi_remove,
	.driver = {
		.name = "oplus21861_boe_nt37701a_fhd_dsi_cmd",
		.owner = THIS_MODULE,
