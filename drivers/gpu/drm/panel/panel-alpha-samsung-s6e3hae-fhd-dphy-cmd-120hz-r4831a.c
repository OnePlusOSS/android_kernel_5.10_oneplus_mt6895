...
#define s6e3hae_lcm_dcs_write_seq(ctx, seq...) \
({\
	const u8 d[] = { seq };\
...
#define s6e3hae_lcm_dcs_write_seq_static(ctx, seq...) \
({\
	static const u8 d[] = { seq };\
	lcm_dcs_write(ctx, d, ARRAY_SIZE(d));\
})

static inline struct s6e3hae_lcm *panel_to_lcm(struct drm_panel *panel)
{
	return container_of(panel, struct s6e3hae_lcm, panel);
}

#ifdef PANEL_SUPPORT_READBACK
static int lcm_dcs_read(struct s6e3hae_lcm *ctx, u8 cmd, void *data, size_t len)
{
...
static void lcm_panel_get_data(struct s6e3hae_lcm *ctx)
{
	u8 buffer[3] = {0};
	static int ret;
...
static void lcm_dcs_write(struct s6e3hae_lcm *ctx, const void *data, size_t len)
{
	struct mipi_dsi_device *dsi = to_mipi_dsi_device(ctx->dev);
	ssize_t ret;
	char *addr;
...
static void push_table(struct s6e3hae_lcm *ctx, struct LCM_setting_table *table, unsigned int count)
{
	unsigned int i, j;
...
{
	struct s6e3hae_lcm *ctx = panel_to_lcm(panel);
...
static int s6e3hae_lcm_unprepare(struct drm_panel *panel)
{
	struct s6e3hae_lcm *ctx = panel_to_lcm(panel);
	...
	s6e3hae_lcm_dcs_write_seq_static(ctx, 0x28);
	msleep(50);
	s6e3hae_lcm_dcs_write_seq_static(ctx, 0x10);
	msleep(150);
...
static int s6e3hae_lcm_prepare(struct drm_panel *panel)
{
	struct s6e3hae_lcm *ctx = panel_to_lcm(panel);
	int ret;
...
static int s6e3hae_lcm_enable(struct drm_panel *panel)
{
	struct s6e3hae_lcm *ctx = panel_to_lcm(panel);
			...
			struct drm_connector *connector, unsigned int mode)
{
	...
	if (drm_mode_vrefresh(m) == MODE0_FPS && m->hdisplay == HACT_WQHD)
		...
		ret = 1;
#else
	if (drm_mode_vrefresh(m) == MODE2_FPS && m->hdisplay == HACT_FHDP)
		ext->params = &ext_params_fhdp;
	else if (drm_mode_vrefresh(m) == MODE3_FPS && m->hdisplay == HACT_FHDP)
		ext->params = &ext_params_120hz_fhdp;
	else
...
static int s6e3hae_lcm_get_modes(struct drm_panel *panel, struct drm_connector *connector)
{
...
static const struct drm_panel_funcs lcm_drm_funcs = {
	.disable = s6e3hae_lcm_disable,
	.unprepare = s6e3hae_lcm_unprepare,
	.prepare = s6e3hae_lcm_prepare,
	.enable = s6e3hae_lcm_enable,
	.get_modes = s6e3hae_lcm_get_modes,
};
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	ctx = devm_kzalloc(dev, sizeof(struct s6e3hae_lcm), GFP_KERNEL);
	if (!ctx)
		return -ENOMEM;
...
{
	struct s6e3hae_lcm *ctx = mipi_dsi_get_drvdata(dsi);
#if defined(CONFIG_MTK_PANEL_EXT)
...
static const struct of_device_id lcm_of_match[] = {
	{ .compatible = "samsung,s6e3hae_fhd_dphy_120hz_cmd", },
	{ }
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "s6e3hae_fqhd_dphy_120hz_cmd",
		.owner = THIS_MODULE,
...
MODULE_DESCRIPTION("samsung, s6e3hae fqhd dphy oled panel driver");
