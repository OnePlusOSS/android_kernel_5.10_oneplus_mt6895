...
int mtk_drm_setbacklight(struct drm_crtc *crtc, unsigned int level, bool atomic)
{
	...
	struct cmdq_client *client; //gaoxiaolei add for fix thread 6/0 issue timeout
	bool is_frame_mode;
	int index = drm_crtc_index(crtc);
	int ret = 0;
	// #ifdef OPLUS_BUG_STABILITY
	...
	if (!atomic && m_new_pq_persist_property[DISP_PQ_CCORR_SILKY_BRIGHTNESS] &&
		sb_cmdq_handle != NULL) {
	...
	if (cur_fps == 0 && crtc->state)
		cur_fps = drm_mode_vrefresh(&crtc->state->adjusted_mode);
	DDPINFO("%s fps: %d\n", __func__, cur_fps);
...
#endif
{
	...
	DDPINFO("crtc_state:%x, atomic_state:%x, crtc:%x, err:%d\n",
		crtc_state,
		atomic_state,
		crtc, data.err);
	...
	if (mtk_crtc_is_frame_trigger_mode(crtc)) {
		...
		if (disp_helper_get_stage() == DISP_HELPER_STAGE_NORMAL) {
			if (mtk_drm_helper_get_opt(priv->helper_opt,
				...
				if (cur_fps != 60 && mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_PRE_TE)) {
...
void mtk_crtc_stop_event_loop(struct drm_crtc *crtc)
{
	...
	if (!mtk_crtc->event_loop_cmdq_handle) {
		DDPMSG("%s: event_loop already stopped\n", __func__);
		return;
	}
