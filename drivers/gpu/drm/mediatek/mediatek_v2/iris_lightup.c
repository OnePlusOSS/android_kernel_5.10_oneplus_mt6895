...
static bool iris_driver_registered;
static bool iris_driver_unregistered;
...
void iris_esd_init(void)
{
	...
	if (pcfg->panel && pcfg->panel_ext) {
		params = pcfg->panel_ext->params;

		for (index = 0; index < ESD_CHK_NUM; index++) {
			if (params->lcm_esd_check_table[index].cmd == 0)
				break;

			pcfg->esd_chk_val[index] = params->lcm_esd_check_table[index].para_list[0];
			IRIS_LOGV("%s() esd_chk_val[%d] = 0x%x", __func__, index, pcfg->esd_chk_val[index]);
		}
...
{
	struct iris_cfg *pcfg = iris_get_cfg();
	struct iris_setting_info *psetting = iris_get_setting();

	if (!iris_is_chip_supported()){
		IRIS_LOGI("%s() chip not supported, return.", __func__);
		return;
	}
...
bool iris_check_dsc_enable(void)
{
	...
	if (pcfg->panel_ext->params)
		return pcfg->panel_ext->params->dsc_params.enable;
...
int iris_get_vrefresh(void)
{
	...
	return drm_mode_vrefresh(pcfg->dsp_mode);
}
...
bool iris_check_high_res(void)
{
	bool high = false;
#if 0
	int fps = iris_get_vrefresh();
	int vres = iris_get_vres();

	if ((fps == HIGH_FREQ) && (vres == FHD_H))
		high = true;
#endif
	return high;
}
...
static u32 iris_get_clkrate(void)
{
	...
	if (pcfg->panel_ext && pcfg->panel_ext->params)
		return pcfg->panel_ext->params->data_rate;
	return 0;
}


void iris_sync_timing(struct dsi_mode_info *ptiming,
...
void iris_sync_cur_timing(void)
{
	...
	iris_sync_timing(&pcfg->timing, pcfg->dsp_mode);
}
		...
		struct iris_cfg *pcfg, int32_t type)
{
	...
	if (type == IRIS_CONT_SPLASH_NONE)
		pseq = _iris_get_ctrl_seq_addr(pcfg->ctrl_seq, 0);
	else if (type == IRIS_CONT_SPLASH_LK)
		pseq = _iris_get_ctrl_seq_addr(pcfg->ctrl_seq_cs, 0);
...
{
	if (type >= IRIS_DTSI_PIP_IDX_START && type < IRIS_PIP_IDX_CNT)
		return true;
...
{
	if (type >= IRIS_DTSI_PIP_IDX_START && type <= IRIS_DTSI_PIP_IDX_END)
		return true;
...
static int32_t _iris_get_ip_idx_type(const struct iris_ip_index *pip_index)
{
	...
	for (type = IRIS_DTSI_PIP_IDX_START; type < IRIS_PIP_IDX_CNT; type++) {
...
static int32_t _iris_alloc_desc_buf(struct iris_cmd_desc **cmds, int cmd_cnt)
{
	int cmd_size = 0;
	...
	cmd_size = cmd_cnt * sizeof(struct iris_cmd_desc);
...
static int32_t _iris_alloc_cmd_buf(struct iris_cmd_desc **cmds,
...
static int32_t _iris_write_ip_opt(struct iris_cmd_desc *cmd,
		...
		struct iris_ip_index *pip_index)
{
	...
	if (i == cnt) {
		IRIS_LOGE("%s(), find ip_ opt fail, ip_ = 0x%02x opt = 0x%02x.",
				__func__, ip, opt_id);
		return -EINVAL;
	}
...
static int32_t _iris_trans_section_hdr_to_desc(
		struct iris_cmd_desc *cmd, const struct iris_cmd_header *hdr)
{
	memset(cmd, 0, sizeof(struct iris_cmd_desc));
...
static int _iris_write_cmd_hdr(struct iris_cmd_desc *cmd,
...
static int _iris_write_cmd_payload(struct iris_cmd_desc *pdesc,
...
struct iris_ip_opt *iris_find_specific_ip_opt(uint8_t ip, uint8_t opt_id, int32_t type)
{
	...
	IRIS_LOGV("%s(), ip_: %#x, opt: %#x, type: %d", __func__, ip, opt_id, type);
	if (!_iris_is_valid_ip(ip)) {
		IRIS_LOGE("%s(), ip_ %d is out of range", __func__, ip);
		return NULL;
	}
...
struct iris_ip_opt *iris_find_ip_opt(uint8_t ip, uint8_t opt_id)
{
	...
	IRIS_LOGV("%s(), ip_: %#x, opt: %#x", __func__, ip, opt_id);
	if (!_iris_is_valid_ip(ip)) {
		IRIS_LOGE("%s(), ip_ %d is out of range", __func__, ip);
		return NULL;
	}
...
static void _iris_print_ipopt(struct iris_ip_index  *pip_index)
{
	...
	for (i = 0; i < ip_cnt; i++) {
		for (j = 0; j < pip_index[i].opt_cnt; j++) {
			...
			IRIS_LOGI("%s(%d), ip_: %02x, opt: %02x, cmd: %p, len: %d, link state: %#x",
					__func__, __LINE__,
...
static int32_t _iris_add_cmd_to_ipidx(const struct iris_data *data,
		struct iris_cmd_desc *cmds, int cmd_pos, struct iris_ip_index *pip_index)
{
	int32_t span = 0;
	int32_t pkt_cnt = 0;
		...
		struct iris_ip_index *pip_index, int32_t cmd_cnt)
{
	...
	int32_t rc = 0;
	int32_t cmd_pos = 0;
	struct iris_cmd_desc *cmds = NULL;
		...
		struct device_node *np, struct iris_cfg *pcfg)
{
	...
	IRIS_LOGI("pxlw,chip-version: %#x", pcfg->chip_ver);
		...
		struct device_node *np, struct iris_cfg *pcfg)
{
	...
	IRIS_LOGI("pxlw,add-last-for-split-pkt: %d, pxlw,add-pt-last-for-split-pkt: %d",
	...
	for (i = 0; i < data_cnt; i++) {
		...
		while (data_len >= sizeof(struct iris_cmd_header)) {
			...
			if (hdr->payload_len > (data_len >> 2)) {
				IRIS_LOGE("%s: length error, ip_ = 0x%02x opt=0x%02x, len=%d",
			...
			IRIS_LOGV("hdr info, type: 0x%02x, last: 0x%02x, wait: 0x%02x, ip_: 0x%02x, opt: 0x%02x, len: %d.",
...
int32_t iris_parse_dtsi_cmd(const struct device_node *lightup_node, uint32_t cmd_index)
{
	...
	if (cmd_index != IRIS_DTSI_PIP_IDX_START)
		snprintf(key, sizeof(key), "pxlw,iris-cmd-list-%d", cmd_index);
		...
		struct iris_cfg *pcfg)
{
	...
	pcfg->rx_mode = mtk_dsi_is_cmd_mode(pcfg->mtk_comp)
			? DSI_OP_CMD_MODE: DSI_OP_VIDEO_MODE;
...
static int _iris_parse_pwr_entries(void)
{
	int32_t rc = 0;
#if 0
...
static void _iris_parse_bl_endian(void)
{

}

static void _iris_parse_i2c_switch_config(
		struct device_node *np, struct iris_cfg *pcfg)
{
	if (!np || !pcfg)
		return;

	pcfg->iris_i2c_switch = of_property_read_bool(np,
			"pxlw,iris-i2c-switch");
	IRIS_LOGI("%s(), iris i2c switch: %s",
			__func__,
			pcfg->iris_i2c_switch ? "true" : "false");
}
...
void iris_get_panel_params(struct drm_device *drm, struct drm_display_mode **dsp_mode)
{
	...
	IRIS_LOGI("crtc num1=%d num =%d\n", drm->num_crtcs, drm->mode_config.num_crtc);
...
int iris_parse_param(void *dev)
{
	...
	iris_get_panel_params(drm, &pcfg->dsp_mode);
	...
	rc = iris_parse_dtsi_cmd(lightup_node, IRIS_DTSI_PIP_IDX_START);
	if (rc) {
...
static void _iris_reset_out_cmds(void)
{
	...
	memset(pcfg->iris_cmds.iris_cmds_buf, 0x00,
			sum * sizeof(struct iris_cmd_desc));
		...
		int32_t opt_index, struct iris_cmd_comp *pcmd_comp)
{
	...
	if (!_iris_is_valid_ip(ip)) {
		IRIS_LOGE("%s(), invalid ip_: %#x", __func__, ip);
		return -EINVAL;
	}

	opt = iris_find_ip_opt(ip, opt_index);
	if (!opt) {
		IRIS_LOGE("%s(), can not find popt, ip_: %#x, opt: %#x",
...
void iris_print_desc_cmds(struct iris_cmd_desc *p, int num, int state)
{
	int i = 0;
	int j = 0;
	int len = 0;
	...
	uint8_t *ptr = NULL;
	uint8_t *ptr_tx = NULL;
	struct iris_cmd_desc *pcmd = NULL;
	...
	for (i = 0; i < num; i++) {
        ...
        len = 6 * dlen + 100; //3* 7(dchdr) + 1(\n) + 1 (0)
        arr = (uint8_t *)kvzalloc(len * sizeof(uint8_t), GFP_KERNEL);
        if (!arr) {
        ...
        len = sprintf(ptr, "{0x%02X,", ptr_tx[0]);
        ptr += len;
        len = sprintf(ptr, " %d, {", dlen - 1);
        ptr += len;
        for (j = 1; j < dlen; j++) {
            if (j%160 == 0){
                sprintf(ptr, "\n");
            ...
            if (j < dlen - 1)
                len = sprintf(ptr, "0x%02X, ", ptr_tx[j]);
            else
                len = sprintf(ptr, "0x%02X} },\n", ptr_tx[j]);
            ptr += len;
        }
        IRIS_LOGE("%s",arr);
        if (pcmd->post_wait_ms > 0) {
            IRIS_LOGE("{REGFLAG_UDELAY, %d, {} },\n", pcmd->post_wait_ms);
        }
		kvfree(arr);
		arr = NULL;
	}
...
static void _iris_print_spec_cmds(struct iris_cmd_desc *p, int cmd_cnt)
{
	int i = 0;
	int j = 0;
	int value_count = 0;
...
static void _iris_print_dtsi_cmds_for_lk(struct iris_cmd_desc *cmds,
...
static int32_t _iris_i2c_send_ocp_cmds(
...
static int32_t _iris_i2c_s_send_ocp_cmds(struct iris_cmd_comp *pcmd_comp)
{
	int i = 0;
	int len = 0;
	uint32_t *payload = NULL;
	...
	IRIS_LOGD("use %s", __func__);
	return 0;
}

static void iris_change_header(struct iris_cmd_comp *pcmd_comp)
{
	int i = 0;
...
static int32_t _iris_dsi_send_ocp_cmds(struct iris_cmd_comp *pcmd_comp,
		int need_lock, struct cmdq_pkt *cmdq_handle)
{
	int ret = 0;
	uint32_t wait = 0;
	struct iris_cmd_desc *cmd = NULL;
	...
	ret = iris_dsi_send_cmds_lockcmdq(pcmd_comp->cmd, pcmd_comp->cnt, pcmd_comp->link_state,
			need_lock, cmdq_handle);
	if (wait)
		udelay(wait);
...
static int32_t __iris_send_cmds(
		...
		struct cmdq_pkt *cmdq_handle)
{
	...
	IRIS_ATRACE_BEGIN("__iris_send_cmds");
	if (path == PATH_DSI)
		ret = _iris_dsi_send_ocp_cmds(pcmd_comp, need_lock, cmdq_handle);
	else if (path == PATH_I2C)
		ret = _iris_i2c_send_ocp_cmds(pcmd_comp);
	else if (path == PATH_I2C_S)
		ret = _iris_i2c_s_send_ocp_cmds(pcmd_comp);
	else
		ret = -EINVAL;
	IRIS_ATRACE_END("__iris_send_cmds");

	return ret;
}

static int32_t _iris_send_cmds(
		struct iris_cmd_comp *pcmd_comp, uint8_t path)
{
	int need_lock = 1;
	struct cmdq_pkt *cmdq_handle = NULL;

	return __iris_send_cmds(pcmd_comp, path, need_lock, cmdq_handle);
}

static int32_t _iris_send_ipopt_cmds(int32_t ip, int32_t opt_id, int need_lock)
{
	int32_t rc = 0;
	struct iris_cmd_comp cmd_comp;
	struct cmdq_pkt *cmdq_handle = NULL;

	IRIS_LOGD("%s(), ip_: %#x, opt: %#x.", __func__, ip, opt_id);
	rc = _iris_init_cmd_comp(ip, opt_id, &cmd_comp);
	if (rc) {
		IRIS_LOGE("%s(), can not find in seq for ip_: 0x%02x opt: 0x%02x.",
				__func__, ip, opt_id);
		return rc;
	}
	if (need_lock)
		need_lock = (cmd_comp.cnt > 1)  ?  0 : need_lock;
	rc = __iris_send_cmds(&cmd_comp, PATH_DSI, need_lock, cmdq_handle);
	return rc;
}

int32_t iris_send_ipopt_cmds(int32_t ip, int32_t opt_id)
{
	return _iris_send_ipopt_cmds(ip, opt_id, 1);
}

int32_t iris_send_ipopt_cmds_nonlock(int32_t ip, int32_t opt_id)
{
	return _iris_send_ipopt_cmds(ip, opt_id, 0);
}
...
int32_t iris_send_ipopt_cmds_i2c(int32_t ip, int32_t opt_id)
{
	...
	return _iris_send_cmds(&cmd_comp, PATH_I2C_S);
}
...
static void _iris_add_last_pkt(struct iris_cmd_desc *cmd, int cmd_cnt)
{
	_iris_update_desc_last(cmd, cmd_cnt, false);
	_iris_update_desc_last(cmd + cmd_cnt - 1, 1, true);
}

static void _iris_update_mult_pkt_last(struct iris_cmd_desc *cmd,
...
static int _iris_set_pkt_last(struct iris_cmd_desc *cmd,
		int cmd_cnt, int skip_last)
{
	int32_t ret = 0;
	struct iris_cfg *pcfg = iris_get_cfg();
		...
		bool is_update, uint8_t path)
{
	...
	IRIS_LOGD("%s(), ip_: %#x opt: %#x, skip last: %d, update: %s",
	...
	if (cur == prev) {
		IRIS_LOGD("lut table is empty for ip_: %02x opt: %02x",
				popt->ip, opt_id);
		return 0;
	}
		...
		struct iris_ctrl_opt *pip_opt, struct iris_cmd_comp *pcomp, uint8_t path)
{
	...
	IRIS_LOGD("%s(), ip_: %#x opt: %#x, skip last: %d.",
	...
	if (rc) {
		IRIS_LOGE("%s(), invalid ip_: %#x opt: %#x.", __func__, ip, opt_id);
		return -EINVAL;
	}
		...
		struct iris_cmd_comp *pcmd_comp)
{
	...
	for (i = 0; i < seq_cnt; i++) {
		ip = arr[i].ip;
		opt_id = arr[i].opt_id;
		IRIS_LOGV("%s(), ip_:%0x opt:%0x", __func__, ip, opt_id);
		...
		if (rc)
			IRIS_LOGE("%s(), [FATAL ERROR] invalid ip_: %0x opt: %0x", __func__, ip, opt_id);
	}
}
		...
		uint8_t ip, uint8_t opt_old, uint8_t opt_new, uint8_t skip_last)
{
	...
	if (i >= max_cnt) {
		IRIS_LOGE("%s(), no empty space to install ip_: %#x, opt old: %#x, opt new: %#x",
...
int iris_read_chip_id(void)
{
	int ret = 0;
	u32 arr[2] = {0};
	const uint32_t sys_pll_ro_status = 0xf0000018;
	const uint32_t mipi_pll_ro_status = 0xf0000034;
	struct iris_cfg *pcfg = iris_get_cfg();

	arr[0] = sys_pll_ro_status;
	ret = iris_i2c_conver_ocp_read(arr, 1, false);
	if (!ret && ((arr[0] & 0xff) == 1)) {
		arr[0] = mipi_pll_ro_status;
		ret = iris_i2c_conver_ocp_read(arr, 1, false);
		if (!ret && ((arr[0] & 0xff) == 1))
			pcfg->chip_id = 1;
	}
...
void iris_free_ipopt_buf(uint32_t ip_type)
{
	...
	int ip_cnt = IRIS_IP_CNT;
	struct iris_cmd_desc *pdesc_addr = NULL;
...
void iris_load_mcu(void)
{
	...
	iris_ocp_write_mult_vals_nonelock(2, values);
	pcfg->mcu_code_downloaded = true;
}



static void _iris_pre_timing_config(void)
{
	int len = 0;
	struct iris_update_ipopt popt[IP_OPT_MAX];
	bool use_2nd_timing = iris_belongs_to_2nd_timing(&iris_get_cfg()->timing);

	iris_init_ipopt_ip(popt, IP_OPT_MAX);
	//sys pll
	iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_SYS, use_2nd_timing ? 0xA1 : 0xA0, 0x1);
	//dtg
	iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_DTG, use_2nd_timing ? 0x1 : 0x0, 0x1);
	//mipi tx
	iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_TX, use_2nd_timing ? 0x4 : 0x0, 0x1);
	//mipi abp
	len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_RX, use_2nd_timing ? 0xE1 : 0xE0, 0x0);
...
void _iris_read_power_mode(void)
{
...
int iris_enable(struct iris_cmd_set *on_cmds)
{
	...
	if (!pcfg->is_prepared) {
		IRIS_LOGI("%s(), pcfg->is_prepared = %d.", __func__, pcfg->is_prepared);
		goto exit;
	}
	...
	iris_update_cur_timing(&pcfg->timing);
	...
	if (lightup_opt & 0x1) {
		if (on_cmds != NULL)
			rc = iris_dsi_send_cmds(on_cmds->cmds, on_cmds->count, on_cmds->state);
	...
	if (pcfg->loop_back_mode == 1) {
		pcfg->loop_back_mode_res = iris_loop_back_verify(PURE_LOOP_BACK);
		return rc;
	}
	...
	if (prev_mode != PASS_THROUGH_MODE)
		iris_abypass_switch_proc(prev_mode, false, true);
	...
	IRIS_LOGI("%s(), iris takes total %d us, prepare %d us, enter PT %d us,"
			" light up %d us, exit PT %d us. Send panel cmd takes %d us.",
			__func__,
...
static void _iris_send_cont_splash_pkt(uint32_t type)
{
	...
	if (type == IRIS_CONT_SPLASH_LK) {
		...
		iris_lp_preinit();
		pcfg->iris_initialized = false;
		pt_mode = iris_abypass_switch_proc(PASS_THROUGH_MODE, false, true);
		if (pt_mode)
			iris_set_out_frame_rate(iris_get_vrefresh());
...
void iris_send_cont_splash(void)
{
	...
	if (!first_enter) {
		type = iris_get_cont_type_with_timing_switch();

		if (lightup_opt & 0x1)
			type = IRIS_CONT_SPLASH_NONE;
		iris_ddp_mutex_lock();
		iris_dsi_pre_cmd(pcfg->mtk_comp, pcfg->crtc);
		_iris_send_cont_splash_pkt(type);
		iris_dsi_pos_cmd(pcfg->mtk_comp, pcfg->crtc);
...
int iris_lightoff(struct iris_cmd_set *off_cmds)
{
	...
	if ((lightup_opt & 0x10) == 0)
		pcfg->abypss_ctrl.abypass_mode = iris_get_default_work_mode(); //clear to ABYP mode

	IRIS_LOGI("%s(%d), mode: %s(%d) ---", __func__, __LINE__,
			iris_is_pt_mode() ? "PT" : "ABYP",
			pcfg->abypss_ctrl.abypass_mode);
	if (off_cmds) {
		if (iris_is_pt_mode())
			iris_pt_send_panel_cmd(off_cmds);
		else
			iris_dsi_send_cmds(off_cmds->cmds, off_cmds->count, off_cmds->state);
	}
		...
		struct iris_cmd_comp *pasm_comp, uint8_t path)
{
	...
	if (rc) {
		IRIS_LOGE("%s(), [FATAL ERROR] invalid ip_: %#x, opt: %#x",
				__func__,
				ip, ctrl_opt.opt_id);
	}
}
...
static int _iris_update_pq_seq(struct iris_update_ipopt *popt, int ipopt_cnt)
{
	...
	for (i = 0; i < ipopt_cnt; i++) {
		...
		if (popt[i].opt_new != popt[i].opt_old) {
			...
			if (j == pseq->cnt) {
				IRIS_LOGE("%s(), failed to find ip_: %#x opt: %d",
						__func__,
...
static struct iris_cmd_desc *_iris_get_desc_from_ipopt(uint8_t ip, uint8_t opt_id, int32_t pos)
{
	struct iris_ip_opt *popt = NULL;
	struct iris_cfg *pcfg = NULL;
	...
	if (popt == NULL) {
		IRIS_LOGE("%s(), can't find ip opt for ip_: 0x%02x, opt: 0x%02x.",
				__func__, ip, opt_id);
		return NULL;
	}
		...
		int32_t pos, uint32_t value)
{
	...
	//if (ip != IRIS_IP_PWIL && ip != IRIS_IP_DPORT)
	//	return;

	for (cmd_list_index = IRIS_DTSI_PIP_IDX_START; cmd_list_index <= IRIS_DTSI_PIP_IDX_END; cmd_list_index++) {
		if (cmd_list_index == iris_get_cmd_list_index())
			continue;
		if (!iris_same_resolution_diff_refresh(cmd_list_index))
			continue;
		popt = iris_find_specific_ip_opt(ip, opt_id, cmd_list_index);
		if (popt == NULL) {
			IRIS_LOGW("%s(), can't find ip_: 0x%02x opt: 0x%02x, from cmd_list_index: %d",
					__func__, ip, opt_id, cmd_list_index);
			continue;
		}
		pdesc = popt->cmd + (pos * 4 - IRIS_OCP_HEADER_ADDR_LEN) / pcfg->split_pkt_size;
		pvalue = (uint32_t *)((uint8_t *)pdesc->msg.tx_buf + (pos * 4) % pcfg->split_pkt_size);
		pvalue[0] = value;
		IRIS_LOGD("sync cmd_list_index: %d,  ip_: %d, opt_id: %x, pos: %d, value: %x",
				cmd_list_index, ip, opt_id, pos, value);
	}
}
		...
		struct iris_update_regval *pregval, bool is_commit)
{
	...
	popt = iris_find_ip_opt(ip, opt_id);
	if (popt == NULL) {
		IRIS_LOGE("%s(), can't find ip_: 0x%02x opt: 0x%02x",
				__func__, ip, opt_id);
		return;
	...
	if (pcfg && pcfg->panel_pending == step) {
		...
		if (step <= 2) {
			if (c_conn->ops.set_backlight)
				rc = c_conn->ops.set_backlight(&c_conn->base, display, bl_lvl);
		...
		if (c_conn->bl_device)
			c_conn->bl_device->props.brightness = brightness;
		pcfg->panel_pending = 0;
...
static ssize_t _iris_chip_id_write(struct file *file, const char __user *buff,
		size_t count, loff_t *ppos)
{
	...
	pcfg->chip_id = val & 0x03;
	iris_init_abyp_ops();

	IRIS_LOGI("%s(%d), chip_id = %d", __func__, __LINE__, pcfg->chip_id);
	return count;
		...
		const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	IRIS_LOGW("%s(), for ip_: 0x%02X opt: 0x%02X, payload[%d] is: 0x%08X",
...
static int _iris_dbgfs_cont_splash_init(void)
{
	...
	if (debugfs_create_file("iris_cont_splash", 0644, pcfg->dbg_root, NULL,
	...
	if (debugfs_create_file("iris_split_pkt", 0644, pcfg->dbg_root, NULL,
	...
	if (debugfs_create_file("chip_id", 0644, pcfg->dbg_root, NULL,
	...
	if (debugfs_create_file("power_mode", 0644, pcfg->dbg_root, NULL,
	...
	if (debugfs_create_file("iris_i2c_srw",	0644, pcfg->dbg_root, NULL,
	...
	if (debugfs_create_file("iris_list_debug",	0644, pcfg->dbg_root, NULL,
	...
	if (debugfs_create_file("iris_cmd_payload", 0644, pcfg->dbg_root, NULL,
...
void iris_prepare(void)
{
	...
	if (!iris_boot) {
		...
		iris_boot = true;	/* prepare ok */
	}
	pcfg->is_prepared = true;
}
