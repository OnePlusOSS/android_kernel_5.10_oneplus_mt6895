...
static void mtk_dsi_lane_num(struct mtk_dsi *dsi)
{
	...
	mtk_dsi_mask(dsi, DSI_TXRX_CTRL, LANE_NUM, tmp_reg);
}
	...
	if (panel_funcs->doze_get_mode_flags) {
		...
		if (mtk_dsi_is_cmd_mode(&dsi->ddp_comp)) {
			...
			if (prj_id == 22021 || prj_id == 22221) {
				writel(0x00010200, dsi->regs + DSI_TXRX_CTRL);
...
/*#ifdef OPLUS_FEATURE_DISPLAY*/
	if (dsi->ddp_comp.id == DDP_COMPONENT_DSI1 && dsi1_enabled == 0) {
		s32 tmp;
		int loop_cnt = 0;
...
/* #endif */ /* OPLUS_FEATURE_ONSCREENFINGERPRINT */
	...
	case DSI_SET_BL:
	{
		...
		if (panel_ext && panel_ext->funcs
			...
			DISP_INFO("DSI_SET_BL: backlight=%d\n", *(int *)params);
...
static int mtk_dsi_probe(struct platform_device *pdev)
{
	...
	if (dsi->ddp_comp.id == DDP_COMPONENT_DSI0) {
		...
		/*#ifdef OPLUS_FEATURE_DISPLAY*/
		if (dsi->ext) {
			dsi->ext->is_connected = panel_connection_from_atag();
			DDPPR_ERR("dsi0 lcm connect %d\n", dsi->ext->is_connected);
		}
		/*#endif*/
#endif
	}
