...
int iris_send_cmdq_cmds(struct iris_cmd_set *pset, int need_lock, struct cmdq_pkt *cmdq_handle)
{
	...
	if (!cmdq_handle) {
		...
		if (pset) {
			...
			while (div_count) {
				iris_create_cmdq_handle(&temp_handle, &is_frame_mode);
		...
		if (pset) {
			for (i = 0; i < count; i++) {
				...
				iris_get_mtk_dsi_op()->transfer(pcfg->mtk_comp,
						cmdq_handle, (void *)pdesc->msg.tx_buf,pdesc->msg.tx_len);
			}
		}
	}
		...
		struct cmdq_pkt *cmdq_handle)
{
	...
	else if (pcfg->valid == PARAM_LIGHTUP) {
...
int iris_send_cmdq_rdcmd(struct iris_cmd_set *pset)
{
	...
	iris_create_cmdq_handle(&cmdq_handle, &is_frame_mode);
	//transfer rd cmd
