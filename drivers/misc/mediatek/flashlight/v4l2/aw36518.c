...
#define AW36518_REG_CHIP_ID               0x00
#define AW36518_REG_ENABLE                0x01
#define AW36518_REG_IVFM                  0x02
#define AW36518_REG_LED_FLASH_BRIGHTNESS  0x03
#define AW36518_REG_LED_TORCH_BRIGHTNESS  0x05
#define AW36518_REG_BOOST_CONF            0x07
...
#define AW36518_REG_LAST_FLASH            0x0D
/* AW36518A DEFINED ONLY */
#define AW36518_REG_VERSION_ID            0x25
#define AW36518_REG_ADJUST                0x31
#define AW36518_REG_WPRT                  0x69

/* bit 0-1 */
#define AW36518_MASK_ENABLE_LED           0x03
#define AW36518_ENABLE_OFF                0x00
#define AW36518_ENABLE_ON                 0x03
/* bit 2-3 */
#define AW36518_MASK_ENABLE_MODE          0x0C
#define AW36518_ENABLE_MODE_STANDBY       0x00
#define AW36518_ENABLE_MODE_IR_DRIVE      0x04
#define AW36518_ENABLE_MODE_TORCH         0x08
#define AW36518_ENABLE_MODE_FLASH         0x0c
/* bit 4-7 */
#define AW36518_ENABLE_TORCH_ON           0x10
#define AW36518_ENABLE_STROBE_ON          0x20
#define AW36518_ENABLE_STROBE_TYPE_EDGE   0x40  // default level triggered
#define AW36518_ENABLE_TX_PIN_ON          0x80

/* Fault Mask */
#define AW36518_FAULT_1_FLASH_TIMEOUT     (1 << 0)
#define AW36518_FAULT_1_UVLO              (1 << 1)
#define AW36518_FAULT_1_THERMAL_SHUTDOWN  (1 << 2)
#define AW36518_FAULT_1_CURRENT_LIMIT     (1 << 3)
#define AW36518_FAULT_1_LED_SHORT_1       (1 << 4)
#define AW36518_FAULT_1_LED_SHORT_2       (1 << 5)
#define AW36518_FAULT_1_VOUT_SHORT        (1 << 6)
#define AW36518_FAULT_1_TX                (1 << 7)
#define AW36518_FAULT_2_OVP               (1 << 1)
#define AW36518_FAULT_2_IVFM_TRIP         (1 << 2)
...
/* 40ms~1.6s, 16 levels*/
#define AW36518_FLASH_TOUT_MIN 200
#define AW36518_FLASH_TOUT_STEP 200
#define AW36518_FLASH_TOUT_MAX 1600
#define AW36518_FLASH_TOUT_DEFAULT 600
...
#define AW36518_LEVEL_TORCH 15
#define AW36518_LEVEL_NUM   32
#define AW36518_LEVEL_FLASH AW36518_LEVEL_NUM

static const int aw36518_current[AW36518_LEVEL_NUM] = {
...
enum aw36518_ic_version {
	FLASHLIGHT_IC_AW36518 = 0,
	FLASHLIGHT_IC_AW36518A,
...
struct aw36518_flash {
	...
	struct pinctrl *aw36518_hwen_pinctrl;
	struct pinctrl_state *aw36518_hwen_high;
	struct pinctrl_state *aw36518_hwen_low;
#if IS_ENABLED(CONFIG_MTK_FLASHLIGHT)
	struct flashlight_device_id flash_dev_id[AW36518_LED_MAX];
#endif
	int level;
	enum aw36518_ic_version ic_version;
};
 ...
 *****************************************************************************/
static int aw36518_pinctrl_init(struct aw36518_flash *flash)
{
	int ret = 0;

	/* get pinctrl */
	flash->aw36518_hwen_pinctrl = devm_pinctrl_get(flash->dev);
	if (IS_ERR(flash->aw36518_hwen_pinctrl)) {
		pr_info("Failed to get flashlight pinctrl.\n");
		ret = PTR_ERR(flash->aw36518_hwen_pinctrl);
		return ret;
	}

	/* Flashlight HWEN pin initialization */
	flash->aw36518_hwen_high = pinctrl_lookup_state(
			flash->aw36518_hwen_pinctrl,
			AW36518_PINCTRL_STATE_HWEN_HIGH);
	if (IS_ERR(flash->aw36518_hwen_high)) {
		pr_info("Failed to init (%s)\n",
			AW36518_PINCTRL_STATE_HWEN_HIGH);
		ret = PTR_ERR(flash->aw36518_hwen_high);
	}
	flash->aw36518_hwen_low = pinctrl_lookup_state(
			flash->aw36518_hwen_pinctrl,
			AW36518_PINCTRL_STATE_HWEN_LOW);
	if (IS_ERR(flash->aw36518_hwen_low)) {
		pr_info("Failed to init (%s)\n", AW36518_PINCTRL_STATE_HWEN_LOW);
		ret = PTR_ERR(flash->aw36518_hwen_low);
	}

	return ret;
}

static int aw36518_pinctrl_set(struct aw36518_flash *flash, int pin, int state)
{
	int ret = 0;
	...
	switch (pin) {
	case AW36518_PINCTRL_PIN_HWEN:
		if (state == AW36518_PINCTRL_PINSTATE_LOW &&
				!IS_ERR(flash->aw36518_hwen_low))
			pinctrl_select_state(flash->aw36518_hwen_pinctrl,
					flash->aw36518_hwen_low);
		else if (state == AW36518_PINCTRL_PINSTATE_HIGH &&
				!IS_ERR(flash->aw36518_hwen_high))
			pinctrl_select_state(flash->aw36518_hwen_pinctrl,
					flash->aw36518_hwen_high);
		else
...
static int aw36518_check_ic_version(struct aw36518_flash *flash)
{
	int rval = EINVAL;
	unsigned int reg_val = 0;

	rval = regmap_read(flash->regmap, AW36518_REG_DEVICE_ID, &reg_val);
	pr_debug("[%s]: FLASHLIGHT_DEVICE_ID = 0x%x\n", __func__, rval);
	rval = regmap_read(flash->regmap, AW36518_REG_VERSION_ID, &reg_val);
	pr_debug("[%s]: FLASHLIGHT_VERSION_ID = 0x%x\n", __func__, rval);
	if (rval < 0) {
		/* addr not available, recognized as AW36518 */
		pr_debug("[%s]: FLASHLIGHT_IC_VERSION = FLASHLIGHT_IC_AW36518\n", __func__);
		flash->ic_version = FLASHLIGHT_IC_AW36518;
	} else if (reg_val & 0x04) {
		pr_debug("[%s]: FLASHLIGHT_IC_VERSION = FLASHLIGHT_IC_AW36518A\n", __func__);
		flash->ic_version = FLASHLIGHT_IC_AW36518A;
	} else {
		pr_debug("[%s]: FLASHLIGHT_IC_VERSION = FLASHLIGHT_IC_AW36518\n", __func__);
		flash->ic_version = FLASHLIGHT_IC_AW36518;
	}
...
static int aw36518_mode_ctrl(struct aw36518_flash *flash)
{
	...
	if (FLASHLIGHT_IC_AW36518A == flash->ic_version) {
		/* set internal sampling delay */
		rval = regmap_update_bits(flash->regmap, AW36518_REG_ADJUST, 0xff, 0x02);
		if (rval < 0) return rval;
		rval = regmap_update_bits(flash->regmap, AW36518_REG_WPRT, 0xff, 0x0C);
		if (rval < 0) return rval;
	}
	...
	case V4L2_FLASH_LED_MODE_NONE:
		/* turn off */
		rval = regmap_update_bits(flash->regmap, AW36518_REG_ENABLE,
			AW36518_MASK_ENABLE_MODE, AW36518_ENABLE_MODE_STANDBY);
		break;
	case V4L2_FLASH_LED_MODE_TORCH:
		/* torch mode */
		rval = regmap_update_bits(flash->regmap, AW36518_REG_ENABLE,
			AW36518_MASK_ENABLE_MODE, AW36518_ENABLE_MODE_TORCH);
		break;
	case V4L2_FLASH_LED_MODE_FLASH:
		/* flash mode */
		rval = regmap_update_bits(flash->regmap, AW36518_REG_ENABLE,
			AW36518_MASK_ENABLE_MODE, AW36518_ENABLE_MODE_FLASH);
		break;
	}
	return rval;
}
			      ...
			      enum aw36518_led_id led_no, bool on)
{
	...
	regmap_read(flash->regmap, AW36518_REG_FLAG1, &reg_val_0a);
	regmap_read(flash->regmap, AW36518_REG_FLAG2, &reg_val_0b);
	pr_info("[%s] read reg 0x0A = 0x%x, 0x0B = 0x%x\n",__func__,reg_val_0a,reg_val_0b);

		if (on) {
			/* enable led 0*/
			rval = regmap_update_bits(flash->regmap, AW36518_REG_ENABLE,
					AW36518_MASK_ENABLE_LED, AW36518_ENABLE_ON);
		} else {
			/* disable led 0*/
			rval = regmap_update_bits(flash->regmap, AW36518_REG_ENABLE,
					AW36518_MASK_ENABLE_LED, AW36518_ENABLE_OFF);
		}
	}
	return rval;
}
				 ...
				 enum aw36518_led_id led_no, unsigned int brt)
{
	...
	if (brt > AW36518_TORCH_BRT_MAX)
		brt = AW36518_TORCH_BRT_MAX;

	br_bits = AW36518_TORCH_BRT_uA_TO_REG(brt);
	if (led_no == AW36518_LED0)
		rval = regmap_update_bits(flash->regmap,
					  AW36518_REG_LED_TORCH_BRIGHTNESS, 0xff, br_bits);
	return rval;
}
				 ...
				 enum aw36518_led_id led_no, unsigned int brt)
{
	...
	if (brt < AW36518_FLASH_BRT_MIN)
		return aw36518_enable_ctrl(flash, led_no, false);
	if (brt > AW36518_FLASH_BRT_MAX)
		brt = AW36518_FLASH_BRT_MAX;

	br_bits = AW36518_FLASH_BRT_uA_TO_REG(brt);
	if (led_no == AW36518_LED0)
		rval = regmap_update_bits(flash->regmap,
					  AW36518_REG_LED_FLASH_BRIGHTNESS, 0xff, br_bits);
	return rval;
}
				...
				unsigned int tout)
{
	...
	rval = regmap_update_bits(flash->regmap,
					  AW36518_REG_TIMING_CONF, 0x0f, tout_bits);
	return rval;
}
...
static int aw36518_get_ctrl(struct v4l2_ctrl *ctrl, enum aw36518_led_id led_no)
{
	...
	if (ctrl->id == V4L2_CID_FLASH_FAULT) {
		...
		rval = regmap_read(flash->regmap, AW36518_REG_FLAG1, &reg_val);
		if (rval < 0)
			goto out;
		if (reg_val & AW36518_FAULT_1_LED_SHORT_1)
			fault |= V4L2_FLASH_FAULT_SHORT_CIRCUIT;
		if (reg_val & AW36518_FAULT_1_LED_SHORT_2)
			fault |= V4L2_FLASH_FAULT_SHORT_CIRCUIT;
		if (reg_val & AW36518_FAULT_1_THERMAL_SHUTDOWN)
			fault |= V4L2_FLASH_FAULT_OVER_TEMPERATURE;
		if (reg_val & AW36518_FAULT_1_FLASH_TIMEOUT)
...
static int aw36518_set_ctrl(struct v4l2_ctrl *ctrl, enum aw36518_led_id led_no)
{
	...
	case V4L2_CID_FLASH_LED_MODE:
		...
		if (flash->led_mode == V4L2_FLASH_LED_MODE_NONE)
			...
			aw36518_enable_ctrl(flash, led_no, true);
		break;
...
static int aw36518_init(struct aw36518_flash *flash)
{
	...
	aw36518_pinctrl_set(flash, AW36518_PINCTRL_PIN_HWEN, AW36518_PINCTRL_PINSTATE_HIGH);

	aw36518_check_ic_version(flash);
	...
	rval = regmap_update_bits(flash->regmap,
				  AW36518_REG_LED_TORCH_BRIGHTNESS, 0xff, 0x00);
	if (rval < 0)
		return rval;
	rval = regmap_update_bits(flash->regmap,
				  AW36518_REG_LED_FLASH_BRIGHTNESS, 0xff, 0x00);
	if (rval < 0)
		return rval;
...
{
	aw36518_pinctrl_set(flash,
			AW36518_PINCTRL_PIN_HWEN, AW36518_PINCTRL_PINSTATE_LOW);
...
static void aw36518_work_disable(struct work_struct *data)
{
	...
	aw36518_uninit(aw36518_flash_data);
}
...
{
	pr_info("aw36518_flash_test open\n");
	return 0;
}

static int aw36518_flash_release(void)
{
	pr_info("aw36518_flash_test release\n");
	return 0;
}
...
{
	aw36518_flash_data->level = aw36518_verify_level(level);
	if (!aw36518_is_torch(aw36518_flash_data->level)) {
		aw36518_torch_brt_ctrl(aw36518_flash_data, channel,
				1000 * aw36518_current[aw36518_flash_data->level]);
	} else {
		aw36518_flash_brt_ctrl(aw36518_flash_data, channel,
				1000 * aw36518_current[aw36518_flash_data->level]);
	}
	return 0;
}

static int aw36518_operate(int channel, int enable)
{
	ktime_t ktime;
	unsigned int s;
	unsigned int ns;

	if (enable && (aw36518_flash_data->level >= 0)) {
		/* set timer */
		if (aw36518_timeout_ms[channel]) {
			s = aw36518_timeout_ms[channel] / 1000;
			ns = aw36518_timeout_ms[channel] % 1000 * 1000000;
		...
		if (!aw36518_is_torch(aw36518_flash_data->level)) {
			...
			aw36518_flash_data->led_mode = V4L2_FLASH_LED_MODE_FLASH;
		}
...
static int aw36518_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	case FLASH_IOC_SET_ONOFF:
		...
		aw36518_operate(channel, (int)fl_arg->arg);
		break;
	...
	case FLASH_IOC_SET_DUTY:
		pr_info("FLASH_IOC_SET_DUTY(%d): %d\n",
				channel, (int)fl_arg->arg);
		aw36518_set_level(channel, (int)fl_arg->arg);
		break;
	...
	case FLASH_IOC_GET_DUTY_CURRENT:
		pr_info("FLASH_IOC_GET_DUTY_CURRENT(%d): %d\n",
				channel, (int)fl_arg->arg);
		fl_arg->arg = aw36518_current[(int)fl_arg->arg];
		break;

	case FLASH_IOC_GET_HW_TIMEOUT:
		pr_debug("FLASH_IOC_GET_HW_TIMEOUT(%d)\n", channel);
		fl_arg->arg = AW36518_FLASH_TOUT_DEFAULT;
		break;
...
{
	pr_debug("aw36518_strobe_store start (%d, %d, %d), (%d, %d)\n",
			arg.type, arg.ct, arg.part,
			arg.level, arg.dur);
	aw36518_set_driver(1);
	aw36518_set_level(arg.channel, arg.level);
	aw36518_timeout_ms[arg.channel] = 0;
	aw36518_operate(arg.channel, 1);

	msleep(arg.dur);

	aw36518_operate(arg.channel, 0);
	aw36518_set_driver(0);
	pr_debug("aw36518_strobe_store end\n");
	return 0;
}
			...
			const struct i2c_device_id *devid)
{
	...
	rval = aw36518_pinctrl_init(flash);
	if (rval < 0)
		return rval;
	...
	aw36518_timeout_ms[AW36518_CHANNEL_CH1] = AW36518_FLASH_TOUT_DEFAULT;
