...
#define DW9718AF_CONTROL_REG			0x00
#define DW9718AF_SET_POSITION_ADDR		0x02
 ...
 */
#define DW9718AF_MOVE_STEPS			100
#define DW9718AF_MOVE_DELAY_US			5000
...
static struct regval_list dw9718af_init_regs[] = {
	{DW9718AF_CMD_DELAY, DW9718AF_CMD_DELAY},
	{DW9718AF_CMD_DELAY, DW9718AF_CMD_DELAY},
};
...
static int dw9718af_release(struct dw9718af_device *dw9718af)
{
	...
	diff_dac = DW9718AF_ORIGIN_FOCUS_POS - dw9718af->focus->val;
	...
	for (i = 0; i < nStep_count; ++i) {
		val += (diff_dac < 0 ? (DW9718AF_MOVE_STEPS*(-1)) : DW9718AF_MOVE_STEPS);
		...
		usleep_range(DW9718AF_MOVE_DELAY_US,
			     DW9718AF_MOVE_DELAY_US + 1000);
	}

	// last step to origin
	ret = dw9718af_set_position(dw9718af, DW9718AF_ORIGIN_FOCUS_POS);
	if (ret) {
		pr_info("%s I2C failure: %d",
			__func__, ret);
		return ret;
...
static int dw9718af_init(struct dw9718af_device *dw9718af)
{
	...
	for (val = dw9718af->focus->val % DW9718AF_MOVE_STEPS;
	     val <= dw9718af->focus->val;
