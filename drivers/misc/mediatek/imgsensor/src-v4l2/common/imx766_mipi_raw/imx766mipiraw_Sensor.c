...
static kal_int32 write_1st_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
	...
	adaptor_i2c_wr_u8(ctx->i2c_client, IMX766_EEPROM_WRITE_ID >> 1, reg, 0x0);

	reg = 0x06ca;
	if (enable) {
		adaptor_i2c_wr_u8(ctx->i2c_client, IMX766_EEPROM_WRITE_ID >> 1, reg, 0x2);
	}
	else {
		adaptor_i2c_wr_u8(ctx->i2c_client, IMX766_EEPROM_WRITE_ID >> 1, reg, 0x0);
	}
				...
				UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
		*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 1981000;
		break;
