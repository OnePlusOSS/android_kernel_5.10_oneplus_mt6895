...
#define PFX "CAM_CAL_S5K3P9SP_21143"
...
static unsigned int do_single_lsc_s5k3p9sp(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_s5k3p9sp(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_s5k3p9sp(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);

static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x01410062, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		{0x00000001, 0x00000000, 0x00000000, do_module_version},
		{0x00000001, 0x00000000, 0x00000002, do_part_number},
		{0x00000001, 0x00000C00, 0x0000134E, do_single_lsc_s5k3p9sp},
		{0x00000001, 0x00000020, 0x0000000E, do_2a_gain_s5k3p9sp},
		...
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_s5k3p9sp}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT s5k3p9sp_op_tri_eeprom_21143 = {
	.name = "s5k3p9sp_op_tri_eeprom_21143",
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	pr_debug("S5K3P9SP block_size=%d sensor_id=%x\n", block_size, pCamCalData->sensorID);
	...
	if (0x1 & AWBAFConfig) {
		...
		debug_log( "5100K AWB\n");
		/* AWB Light Source Calibration Value (5100K) */
		awb_offset = 0x48;
		...
		if (read_data_size > 0)	{
			...
			pr_debug_err("Read CalValue Failed\n");
			show_cmd_error_log(pCamCalData->Command);
		}
		/* AWB Unit Gain (5100K) */
		awb_offset = 0x0C;
		...
		#endif
		/* AWB Light Source Calibration Value (3100K) */
		debug_log( "3100K AWB\n");
		rgCalValue = bgCalValue = 0;
		awb_offset = 0x4C;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				awb_offset, 8, (unsigned char *)&CalValue);
		if (read_data_size > 0)	{
		...
		pr_debug("======================AWB CAM_CAL==================\n");
		#endif
		/* AWB Light Source Calibration Value (4000K) */
		debug_log( "4000K AWB\n");
		rgCalValue = bgCalValue = 0;
		awb_offset = 0x64;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
