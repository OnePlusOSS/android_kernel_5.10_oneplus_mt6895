...
static unsigned int        last_touch_mode = 0;
...
static int oplus_tp_notifier_call(struct notifier_block *nb, unsigned long val, void *data) {
	...
	if (tp_event->touch_state == last_touch_mode) {
		return IRQ_HANDLED;
	}
	...
	if (val == EVENT_ACTION_FOR_FINGPRINT) {
		...
		if (1 == tp_info->touch_state) {
			...
			last_touch_mode = tp_info->touch_state;
...
int opticalfp_irq_handler_uff(struct fp_underscreen_info *tp_info) {
	...
	if (tp_info->touch_state == last_touch_mode) {
		return IRQ_HANDLED;
	}
...
#if defined(CONFIG_OPLUS_FEATURE_LOG_CORE)
