...
static int silfp_set_spi_cs_low(struct silfp_data *fp_dev)
{
	...
	struct pinctrl_state *spi_cs = pinctrl_lookup_state(fp_dev->pin.pinctrl,
			"cs-low");
	...
	LOG_MSG_DEBUG(ERR_LOG, "set spi cs low");
...
static int silfp_parse_dts(struct silfp_data *fp_dev)
{
	...
	if (fp_dev->spi->dev.of_node) {
		/* Get the SPI ID (#1-6) */
		ret = of_property_read_u32(fp_dev->spi->dev.of_node, "spi-id",
				&fp_dev->pin.spi_id);
		...
		ret = of_property_read_u32(fp_dev->spi->dev.of_node, "spi-irq",
				&fp_dev->pin.spi_irq);
		...
		ret = of_property_read_u32(fp_dev->spi->dev.of_node, "spi-reg",
				&fp_dev->pin.spi_reg);
...
static int silfp_set_spi_default_status(struct silfp_data *fp_dev)
{
	int ret = 0;
#ifdef BSP_SIL_CTRL_SPI
	fp_dev->pin.spi_default = pinctrl_lookup_state(fp_dev->pin.pinctrl,
			"spi-default");
...
static int silfp_resource_init(struct silfp_data *fp_dev,
