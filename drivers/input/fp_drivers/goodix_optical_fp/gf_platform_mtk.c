...
#ifdef BSP_GF_FP_EXT_PMIC
extern int fingerprint_ldo_enable(unsigned int, unsigned int);
extern int fingerprint_ldo_disable(unsigned int, unsigned int);
#else
...
int gf_parse_dts(struct gf_dev* gf_dev)
{
	...
	if (FP_GOODIX_5658 == get_fpsensor_type()
		...
		|| FP_GOODIX_3956 == get_fpsensor_type()) { /* add new fpsensor if needed */
	...
	/*get ext_ldo resource*/
	if (of_property_read_bool(node, "gf,enable-external-pmic")) {
		pr_err("%s, Using external pmic \n", __func__);
		gf_dev->gf_ext_pmic_flag = 1;
	} else {
		gf_dev->gf_ext_pmic_flag = 0;
	}

	if (gf_dev->gf_ext_pmic_flag) {
		rc = of_property_read_u32(node, "gf,ext-pmic-ldo-num", &gf_dev->gf_ext_pmic_ldo_num);
		if (rc) {
			pr_err("%s, Unknown ldo number \n", __func__);
			return -1;
		}

		rc = of_property_read_u32(node, "gf,ext-pmic-ldo-mv-max", &gf_dev->gf_ext_pmic_ldo_mv_max);
		if (rc) {
			pr_err("%s, Unknown ldo mv_max \n", __func__);
			return -1;
		}

		rc = of_property_read_u32(node, "gf,ext-pmic-ldo-mv-min", &gf_dev->gf_ext_pmic_ldo_mv_min);
		if (rc) {
			pr_err("%s, Unknown ldo mv_min \n", __func__);
			return -1;
		}

		pr_info("gf_ext_pmic_ldo_num = %d, gf_ext_pmic_ldo_mv_max = %d, gf_ext_pmic_ldo_mv_min = %d\n",
					gf_dev->gf_ext_pmic_ldo_num,
					gf_dev->gf_ext_pmic_ldo_mv_max,
					gf_dev->gf_ext_pmic_ldo_mv_min);
	} else {
...
int gf_power_on(struct gf_dev* gf_dev)
{
	...
	if (gf_dev->gf_ext_pmic_flag == 1) {
		if (fingerprint_ldo_enable(gf_dev->gf_ext_pmic_ldo_num, gf_dev->gf_ext_pmic_ldo_mv_max)) {
			pr_err("%s: enable LDO%d failed \n", __func__, gf_dev->gf_ext_pmic_ldo_num);
			rc = -1;
		} else {
			pr_err("%s: enable LDO%d success \n", __func__, gf_dev->gf_ext_pmic_ldo_num);
		}
	}
...
int gf_power_off(struct gf_dev* gf_dev)
{
    ...
    for (index = 0; index < gf_dev->power_num; index++) {
        ...
        default:
            rc = -1;
			pr_info("---- power off mode not set !!! ----\n");
            break;
        }
	...
	if (gf_dev->gf_ext_pmic_flag == 1) {
		if (fingerprint_ldo_disable(gf_dev->gf_ext_pmic_ldo_num, gf_dev->gf_ext_pmic_ldo_mv_min)) {
			pr_err("%s: disable LDO%d failed \n", __func__, gf_dev->gf_ext_pmic_ldo_num);
			rc = -1;
		} else {
			pr_err("%s: disable LDO%d success \n", __func__, gf_dev->gf_ext_pmic_ldo_num);
		}
	}
